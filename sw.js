...
const CACHE='esteticagyr-v5-png-private-vary-range-safe-shell';
...
function safeResponse(res){if(!res||!res.ok||res.type==='opaque') return false;const cc=(res.headers.get('cache-control')||'').toLowerCase();if(cc.includes('private')||cc.includes('no-store')) return false;if(res.headers.has('set-cookie')||res.headers.has('content-range')) return false;const vary=(res.headers.get('vary')||'').toLowerCase().split(',').map(v=>v.trim());return !vary.some(v=>v==='*'||v==='cookie'||v==='authorization'||v==='range'||v==='if-range')}
